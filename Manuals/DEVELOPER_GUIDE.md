...
│   │   ├── start.py          # Команда /start, проверка подписки
│   │   ├── menu.py           # Главное меню и навигация
│   │   ├── generate.py       # Генерация изображений
│   │   ├── edit.py           # Редактирование изображений (до 10 фото)
...
│   │   └── admin.py          # Состояния админки (рассылка)
...
- Значения по умолчанию (`initial_tokens = 7`)
...
2. **Rate limiting** — ограничение запросов от одного пользователя (уже реализовано через `MAX_TASKS_PER_USER_PER_HOUR`)
...
6. **Fail2ban** — защита от brute-force атак на сервер
7. **Backup автоматизация** — регулярные бэкапы БД в облако
...
**Файлы:** `bot/db/models.py`, `bot/handlers/start.py`, `bot/handlers/invite.py`

Пользователи могут приглашать друзей через реферальные ссылки:
- Формат: `https://t.me/bot_username?start=ref_USERID`
- Поле `referrer_id` в таблице `users`
- Команда `/invite` показывает реферальную ссылку и статистику

### Проверка подписки на канал

**Файлы:** `bot/handlers/start.py`, `bot/config.py`

Новые пользователи должны подписаться на канал для получения токенов:
- Настраивается через `SUBSCRIPTION_CHANNEL` и `SUBSCRIPTION_REQUIRED`
- Бот должен быть админом канала
- Админы могут включать/выключать через `/togglesub`

### Видео-приветствие

**Файл:** `bot/handlers/start.py`

Отправка видео-кружка всем новым пользователям при `/start`:
- Настраивается через `WELCOME_VIDEO_FILE_ID`
- Получить file_id: отправить видео-кружок боту как админ

### Кнопки оценки результата

**Файлы:** `bot/handlers/feedback.py`, `bot/keyboards/inline.py`

После генерации показываются кнопки:
- 👍 — благодарит за отзыв
- 👎 — показывает меню настроек для регенерации
- 🔄 Сгенерировать ещё — открывает экран подтверждения с теми же настройками

### Редактирование через reply

**Файл:** `bot/handlers/edit.py`

Пользователь может ответить текстом на сгенерированное изображение — автоматически запустится flow редактирования.

### Множественные изображения

**Файл:** `bot/handlers/edit.py`

При редактировании можно загрузить до 10 фото:
- Batch-загрузка (media group) обрабатывается одним сообщением
- Динамический счётчик "✅ Получено фото: 3/10"
- Дополнительная стоимость за 4+ фото
...
### Получение file_id для видео

Админы могут получить file_id любого файла просто отправив его боту:
- 📹 Видео-кружки (video_note) — для `WELCOME_VIDEO_FILE_ID`
- 🖼 Фото (photo)
- 🎥 Видео (video)
- 📄 Документы (document)

Бот автоматически ответит с file_id и инструкцией.

**Реализация:** `bot/handlers/admin.py`

```python
@admin_router.message(F.video_note)
async def handle_admin_video_note(message: Message) -> None:
    """Админ отправил видео-кружок — показываем file_id."""
    file_id = message.video_note.file_id
    await message.answer(
        f"📹 <b>Video Note File ID:</b>\n\n"
        f"<code>{file_id}</code>\n\n"
        f"Скопируйте этот ID в переменную <code>WELCOME_VIDEO_FILE_ID</code> в .env файле."
    )
```

### Уведомления админов

**Файл:** `bot/services/admin_notify.py`

Централизованный сервис для отправки уведомлений админам:

```python
from bot.services.admin_notify import (
    notify_admins,
    notify_error,
    notify_moderation_block,
    notify_generation_failure
)

# Общее уведомление
await notify_admins(
    message="Что-то важное произошло",
    title="⚠️ Внимание"
)
...
# Уведомление о блокировке модерацией
await notify_moderation_block(
    user_id=user.id,
...
await notify_generation_failure(
    task_id=task.id,
    user_id=user.id,
    telegram_id=user.telegram_id,
    username=user.username,
...
Ошибки модерации OpenAI обрабатываются специально:
- Не повторяются (no retry)
- Токены возвращаются сразу
- Пользователь получает понятное сообщение
- Админы получают уведомление

```python
class ModerationError(Exception):
    """Ошибка модерации контента OpenAI."""
    pass

async def _send_moderation_notification(task, user):
    """Уведомляет пользователя о блокировке модерацией."""
    await bot.send_message(
        chat_id=user.telegram_id,
        text=(
            "⚠️ <b>Запрос отклонён модерацией</b>\n\n"
            f"К сожалению, ваш запрос не прошёл проверку безопасности OpenAI.\n\n"
            f"Токены (<b>{task.tokens_spent}</b>) возвращены на ваш баланс.\n\n"
            "💡 Попробуйте изменить описание и избегать запрещённого контента."
        )
    )
```

## 🎨 Новая токеновая система

### Цены (пользовательские токены)

| Качество | Стоимость | Описание |
|----------|-----------|----------|
| Low | 2 🪙 | Быстрое |
| Medium | 5 🪙 | Стандарт |
| High | 20 🪙 | Максимум |
...
**Файл:** `bot/services/image_tokens.py`

```python
from bot.services.image_tokens import calculate_user_tokens

# Для генерации
tokens = calculate_user_tokens(quality="medium")  # 5

# Для редактирования с несколькими фото
tokens = calculate_user_tokens(quality="high", images_count=5)  # 20 + 1 = 21
```
...
| `/addtokens <@username\|telegram_id> <amount>` | Добавить токены пользователю |
| `/userinfo <@username\|telegram_id>` | Информация о пользователе |
| `/resetuser <@username\|telegram_id>` | Сбросить пользователя к начальным настройкам |
| `/togglesub` | Включить/выключить проверку подписки |
| `/broadcast` | Рассылка сообщений всем пользователям |

**Пример использования:**
```
/addtokens @username 5000
...
В админ-панели топ пользователей показывает:
```
1. @username (123456789) — 50 задач
2. @another_user (987654321) — 30 задач
3. @— (555555555) — 20 задач
```

### Рассылка

Команда `/broadcast` позволяет отправить сообщение всем пользователям:
1. Запрашивает сообщение (текст/фото/видео)
2. Показывает количество получателей
3. Требует подтверждения
4. Отправляет с прогрессом (обновляется каждые 50 пользователей)
5. Показывает итоги (успешно/ошибки)

**Реализация:** `bot/handlers/admin.py`, `bot/states/admin.py`
...
|------------|--------------|----------|
| `HIGH_COST_THRESHOLD` | 20 | Порог токенов для двойного подтверждения |
...
| Переменная | По умолчанию | Описание |
|------------|--------------|----------|
| `SUBSCRIPTION_CHANNEL` | (пусто) | Канал для проверки подписки (например @nkonshin_ai) |
| `SUBSCRIPTION_REQUIRED` | false | Требовать подписку для новых пользователей |

### Настройки приветствия

| Переменная | По умолчанию | Описание |
|------------|--------------|----------|
| `WELCOME_VIDEO_FILE_ID` | (пусто) | File ID видео-кружка для приветствия |
| `SUPPORT_USERNAME` | (пусто) | Username поддержки (например @support) |
...
# Приветствие
WELCOME_VIDEO_FILE_ID=DQACAgIAAxkBAAI...
